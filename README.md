...
- **Satellite overlay**: transparent PNGs of real spacecraft (Explorer-1, Hubble) cross the viewport slowly in a straight line in a `pointer-events-none` fixed layer above all text at full opacity (no fade — the image enters and exits the screen); the catalog links each to its Commons source and license.
